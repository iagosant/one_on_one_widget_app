class WidgetFormsController < ApplicationController
  ...
  def index
    @widget_forms = WidgetForm.where(user_id: current_user.id)
  end
  ...
  def edit
    ...
    if @widget_form.user != current_user
  ...
  private
    ...
    def widget_form_params
      params.require(:widget_form).permit(:title_text, :background_color, :user_id)
    end
end
